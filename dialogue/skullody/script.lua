...
function onUpdate()
	if song then
		setPropertyFromClass("openfl.Lib", "application.window.title", 'Vs. CDD: Skullody')
	end
	
	if gameOverWindow then
		setPropertyFromClass("openfl.Lib", "application.window.title", 'ay no mames wey es el pinche tutorial')
	end
end
